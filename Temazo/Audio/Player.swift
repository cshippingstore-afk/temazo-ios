...
/// Player basado en iframe oficial de YouTube (vía `IframePlayerEngine` con WKWebView).
/// Cumple ToS de YouTube → views cuentan, royalties al artista, ads cuando los haya
/// (usando youtube-nocookie.com para reducir frecuencia).
///
/// La API pública (playTrack, togglePlay, next, prev, seekTo, stopAll) no cambia
/// — toda la UI sigue funcionando sin modificar.
@MainActor
    ...
    private func hookEngineCallbacks() {
        ...
        engine.onTime = { [weak self] pos, dur in
            Task { @MainActor [weak self] in
                ...
                // Solo aceptar la duración del iframe si el backend no la dió
        ...
        engine.onError = { [weak self] code in
            Task { @MainActor [weak self] in
                ...
                // Errores 100/101/150 = no embeddable / no disponible → siguiente
